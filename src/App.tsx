    ...
    <main className="page">
      <section className="page-title">
        <div>
          ...
          <p>Vehículos, equipamiento, radios y personal desplegado.</p>
        </div>
      ...
      <section className="resource-category-summary">
        {resourceGroups.map(group => {
          ...
          const total = groupResources.reduce(
            (quantity, resource) => quantity + resource.quantity,
            0
          );
          const active = groupResources
            .filter(resource =>
              ["Asignado", "En tránsito", "En operación"].includes(resource.status)
            )
            .reduce((quantity, resource) => quantity + resource.quantity, 0);

          return (
            <article key={group.id}>
              <span>{group.title}</span>
              <strong>{total}</strong>
              <small>{active} activos o asignados</small>
            </article>
          );
        })}
      </section>
            ...
            <section className="resource-group" key={group.id}>
              ...
              {resources.length > 0 ? (
                <div className="resource-board">
                  {resources.map(resource => (
                    <article
                      className={`resource-card status-${slug(resource.status)}`}
                      key={resource.id}
                    >
                      ...
                      <dl>
                        <div>
                          <dt>Responsable</dt>
                          <dd>{resource.responsible || "Sin responsable asignado"}</dd>
                        </div>
                        <div>
                          <dt>Ubicación</dt>
                          <dd>{resource.location || "Sin ubicación"}</dd>
                        </div>
                        <div>
                          <dt>Asignación</dt>
                          <dd>{resource.assignment || "Sin asignación"}</dd>
                        </div>
                        <div>
                          <dt>Cantidad</dt>
                          <dd>{resource.quantity}</dd>
                        </div>
                      </dl>
                      ...
                      <div className="resource-actions">
                        <select
                          ...
                          onChange={event =>
                            changeStatus(resource, event.target.value as ResourceStatus)
                          }
                        >
                          {resourceStatuses.map(status => (
                ...
                <div className="resource-group-empty">
                  <Truck/>
                  <p>No hay recursos registrados en esta categoría.</p>
                </div>
              )}
            </section>
          );
