...
import {
  ...
  ShieldAlert, Trash2, Truck, UserRound, UsersRound, X, Pencil, Search, Archive, Download, Upload, RefreshCw, WifiOff
} from "lucide-react";
import type {
...
function getAvailableQuantity(resource: Resource) {
  if (["Liberado", "Fuera de servicio"].includes(resource.status)) {
    return 0;
  }
...
function getUnavailableQuantity(resource: Resource) {
  if (["Liberado", "Fuera de servicio"].includes(resource.status)) {
...
type SyncState = "synced" | "saving" | "local" | "error" | "offline";
...
function App() {
  ...
  const [syncState, setSyncState] = useState<SyncState>(supabaseConfigured ? "saving" : "local");
  ...
  useEffect(() => {
    const handleOnline = () => {
      setIsOnline(true);
      setSyncState(supabaseConfigured ? "saving" : "local");
    };
    const handleOffline = () => {
      setIsOnline(false);
  ...
  async function syncNow(currentData: AppData = data) {
  ...
  useEffect(() => {
    saveLocal(data);
    setSyncState(!navigator.onLine ? "offline" : supabaseConfigured ? "saving" : "local");
  ...
  function exportBackup() {
    ...
    link.href = url;
    link.download = `sci-${safeFileName(data.incident.name)}-${date}.json`;
  ...
  async function importBackup(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      if (!isValidBackup(parsed)) {
        window.alert("El archivo no corresponde a un respaldo válido de SCI · UGRED.");
        return;
      }

      const confirmed = window.confirm(
        `¿Importar el incidente "${parsed.incident.name}"?\n\nLos datos actuales serán reemplazados. Antes de continuar, conviene exportar el respaldo vigente.`
      );
      if (!confirmed) return;
      ...
      const importedData: AppData = {
        ...
        timeline: [
          ...parsed.timeline,
          {
            ...
            category: "Decisión",
            description: `Respaldo importado desde el archivo ${file.name}.`,
            author: "Sistema"
          }
        ]
      };
      ...
      saveLocal(importedData);
      await syncNow(importedData);
      window.alert("Respaldo importado correctamente.");
    } catch {
      window.alert("No fue posible leer el archivo. Comprueba que sea un respaldo JSON válido.");
    }
  }
  ...
  const syncLabel =
    ...
    syncState === "offline" ? "Sin conexión · solo local" :
  ...
  const summary = useMemo(() => {
    const activeResources = data.resources.reduce(
      (sum, resource) => sum + getDeployedQuantity(resource),
      0
    );
    const availableResources = data.resources.reduce(
      (sum, resource) => sum + getAvailableQuantity(resource),
      0
    );
    ...
    <div className="app-shell">
      <header className="topbar">
        ...
        <div className={`sync sync-${syncState}`}>
          {syncState === "offline" ? <WifiOff size={17}/> : <Database size={17}/>}
          <div>
          ...
          {(syncState === "error" || syncState === "offline") && (
            <button
              ...
              disabled={!isOnline}
              title={isOnline ? "Reintentar sincronización" : "Sin conexión a internet"}
            >
              <RefreshCw/>
            </button>
          )}
      ...
      <section className="backup-toolbar no-print">
        <div>
          <strong>Respaldo del incidente</strong>
          <span>Exporta una copia completa o restaura un archivo JSON.</span>
        </div>
        <div className="backup-actions">
          <button onClick={exportBackup}><Download/>Exportar respaldo</button>
          <button className="secondary-backup" onClick={() => backupInputRef.current?.click()}><Upload/>Importar respaldo</button>
          <input
            ref={backupInputRef}
      ...
      <article className="panel wide">
        ...
        <div className="dashboard-resource-categories">
          {resourceGroups.map(group => {
            ...
            const unavailable = resources.reduce((sum, resource) => sum + getUnavailableQuantity(resource), 0);
            ...
            return (
              <button key={group.id} onClick={() => setTab("recursos")}>
                ...
                <strong>{total}</strong>
                <small>
                  {deployed} desplegados · {available} disponibles
                  {unavailable > 0 ? ` · ${unavailable} no disponibles` : ""}
                </small>
              </button>
            );
          })}
    ...
    <main className="page">
      ...
      <section className="resource-category-summary">
        {resourceGroups.map(group => {
          ...
          const unavailable = groupResources.reduce((sum, resource) => sum + getUnavailableQuantity(resource), 0);
              ...
              {resources.length > 0 ? (
                <div className="resource-board">
                  {resources.map(resource => {
                    ...
                    const unavailable = getUnavailableQuantity(resource);
                    ...
                    return (
                      <article className={`resource-card status-${slug(resource.status)}`} key={resource.id}>
                        ...
                        <div className={`resource-quantity-strip ${unavailable > 0 ? "has-unavailable" : ""}`}>
                          ...
                          {unavailable > 0 && (
                            <div className="quantity-unavailable">
                  ...
                  <tbody>
                    {resources.map(resource => (
                      <tr key={resource.id}>
                        ...
                        <td>{getAvailableQuantity(resource)}</td>
                        <td>{getUnavailableQuantity(resource)}</td>
